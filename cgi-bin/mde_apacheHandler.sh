#!/bin/bash
#
# CGI-script to parse Markdown files with the PHP Markdown Extended class by APACHE direct handling
# Copyright (c) 2004-2014 Pierre Cassat
# Sources at <http://github.com/piwi/html5-quick-template/tree/mde-master>
#

## Config
CONSOLE="`pwd`/markdown-extended/bin/markdown-extended"
CHARSET='utf-8'
OPTIONS="-t=`pwd`/mde-html5-quick-template.html"
PHP_BIN="`which php`"
REQ="$PATH_TRANSLATED"
PLAIN="$QUERY_STRING"
if [ ! -z "$MDE_CHARSET" ]; then CHARSET="$MDE_CHARSET"; fi
if [ ! -z "$MDE_CONSOLE_OPTIONS" ]; then OPTIONS="${OPTIONS}${MDE_CONSOLE_OPTIONS}"; fi
